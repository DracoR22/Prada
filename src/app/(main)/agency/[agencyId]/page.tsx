import CircleProgress from "@/components/global/circle-proggress";
...
const AgencyIdPage = async ({ params }: { params: { agencyId: string }; searchParams: { code: string }}) => {
  ...
  if (agencyDetails.connectAccountId) {
    ...
    .map((session) => ({...session, created: new Date(session.created).toLocaleDateString(),
    ...
    closingRate = + ((totalClosedSessions.length / checkoutSessions.data.length) * 100).toFixed(2) || 0
    }

  return (
    <div className="relative h-full">
      {/* SHOW THIS IF USER HAVENT CONNECTED THEIR STRIPE ACCOUNT */}
        ...
        <div className="flex gap-4 xl:!flex-row flex-col">
          <Card className="p-4 flex-1">
            ...
            <AreaChart className="text-sm stroke-primary"
               data={[
                ...(totalClosedSessions || []),
          ...
          <Card className="xl:w-[400px] w-full">
            ...
            <CardContent>
              <CircleProgress value={closingRate}
                description={
                  <>
                    {sessions && (
