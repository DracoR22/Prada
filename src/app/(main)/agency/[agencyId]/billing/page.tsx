...
const BillingPage = async ({ params }: Props) => {
   ...
   // ADDITIONAL PRODUCTS IN THIS CASE: 'PRIORITY SUPPORT'
   ...
   // GET SUBSCRIPTION
   const { agencySubscription, currentPlanDetails} = await getSubscription(params.agencyId)

   const prices = await stripe.prices.list({
    product: process.env.NEXT_PRADA_PRODUCT_ID,
    active: true
   })
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:!flex-row justify-between gap-8">
        {/* THIS IS A LIST OF ALL AVAILABLE PLANS */}
         ...
         {addOns.data.map((addOn) => (
          <PricingCard
            ...
            highlightDescription="Get priority support and skip those long waiting times with the click of a button."
          />
        ))}
      </div>
      {/* PAYMENT HISTORY */}
