...
import { getFunnel, upsertFunnelPage } from "@/lib/queries"
import { Button } from "../ui/button"
import CustomModal from "../global/custom-modal"
import { useModal } from "@/providers/modal-provider"
import CreateFunnelPage from "../forms/funnel-page-form"
                  ...
                  <ScrollArea className="h-full">
                     ...
                     {pagesState.length ? (
                      <DragDropContext onDragEnd={onDragEnd} onDragStart={onDragStart}>
                        ...
                        //  IF WE HAVE NO PAGES
                        <div className="text-ceenter text-muted-foreground py-6">
                             No Pages
                        </div>
                     )}
                  </ScrollArea>
                            ...
                            <CardDescription className="flex flex-col gap-4">
                               <div className="border-2 rounded-lg sm:w-80 w-full overflow-clip">
                                  ...
                                  <Link href={`/subaccount/${subaccountId}/funnels/${funnelId}/editor/${clickedPage?.id}`} className="relative group">
                                     <div className="cursor-pointer group-hover:opacity-30">
                                        <FunnelPagePlaceholder/>
                                     </div>
                                     <LucideEdit  className="!text-muted-foreground absolute top-1/2 left-1/2 opacity-0 transofrm -translate-x-1/2 -translate-y-1/2 group-hover:opacity-100 transition-all duration-100"/>
                                  </Link>
                                  {/* NAVIGATE TO THE PAGE SUBDOMAIN URL */}
